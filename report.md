...
#####2.2.2  使用可行性
　　这个兼职系统是在调查的基础上针对起航兼职俱乐部开发的，用户是具有社会可行性的。硬件方面，公司有能力配置一定数目的电脑设施，网络已经全部覆盖。软件方面，系统界面良好，便于操作；系统移植性能良好；模块与模块之间相互独立；数据库具有保密性。而且系统的开发也是根据起航兼职俱乐部原有的操作业务和流程开发，操作人员很快就能适应，不必对业务流程进行重大的调整就能运作。另外，系统对用户的权限进行了分层控制，使不同人员既能使用又不越权。
